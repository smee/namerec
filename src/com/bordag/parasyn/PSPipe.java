...
/**
 * takes : grfNAVS : wortNr WortArt GrundFormNummer
 *         de.r_prim_types
 *         de.r_type_names
 */
...
public class PSPipe
{
  ...
  public PSPipe(ParaSynMap data, DBUtil dbUtil, PSDataFactory factory)
  {
    this.data = data;
    this.dbUtil = dbUtil;
...
//    map = importFile();
    if ( map == null )
    {
      ...
      for ( Iterator it = this.data.collocations.iterator(); it.hasNext(); )
      {
        ...
        ParaSynMap curPS = getCurMeaningMap( factory.getPSData( curWord ) );
...
/*    PSWord a = new PSWord(new CHString("a"),new Integer(10), 0.0, 1);
    PSWord ax = new PSWord(new CHString("x"),new Integer(11), 0.0, 1);
    PSWord ay = new PSWord(new CHString("y"),new Integer(12), 0.0, 1);
    PSWord az = new PSWord(new CHString("z"),new Integer(13), 0.0, 1);

  // test data with x,y,z and a
    Set colls = new HashSet();
    colls.add(ax);
    colls.add(ay);
    colls.add(az);
    this.data = new ParaSynMap(colls);
    this.data.setWord(a);
    // first create the colls then add word with setWord
    map = new HashMap();*/
    runAlgorithm(map);
  }
  ...
  /**
   * Todo: Choose the currently fitting meaning map
   * @param maps
   */
  protected ParaSynMap getCurMeaningMap(Vector maps)
  {
    Iterator it = maps.iterator();
    return (ParaSynMap)it.next();
  }
...
//    System.out.println("HY --> "+this.data.getHyperonymsRadian().toStringOrderedBy(0));
  ...
  public static void main(String[] args)
  {
    ...
    try
    {
      System.out.print("Creating connection: to "+url+" : ");
      ...
      for ( Iterator it = factory.getPSData(word).iterator() ; it.hasNext() ; )
      {
        PSPipe pipe = new PSPipe( (ParaSynMap)it.next() , util, factory );
      }
