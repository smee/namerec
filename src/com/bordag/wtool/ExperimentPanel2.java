 ...
 */
public class ExperimentPanel2 extends JPanel
{
  ...
  public ExperimentPanel2(ConnectionPanel cPanel)
  {
    this.cPanel = cPanel;
...
////System.out.println("printing: "+curInfo);
      int x = 0;
      int y = 0;
      x = (int)(ox + curInfo.getX()*this.cSystemSize.getX());
      y = (int)(oy - curInfo.getY()*this.cSystemSize.getY());
      ...
      if ( x > 0 && y > 0 )
      {
        ...
        bufferGraphics.setFont(bufferGraphics.getFont().deriveFont((float)12.0));
        if ( word.length() > 12 )
        {
        ...
        {
          if ( word.equalsIgnoreCase("MEAN") || word.equalsIgnoreCase("COHYPONYM") ||
               word.equalsIgnoreCase("LING COLL") || word.equalsIgnoreCase("HYPERONYM") )
          {
    ...
    if ( this.cPanel.connection != null && word != null && word.length() > 0 )
    {
      try
      {
        ...
        try
        {
          ...
          System.out.println("LC --> "+this.myMap.getLinguisticCollocationsRadian().toStringOrderedBy(0));
          System.out.println("CO --> "+this.myMap.getCohyponymsRadian().toStringOrderedBy(0));
          System.out.println("HY --> "+this.myMap.getHyperonymsRadian().toStringOrderedBy(0));
