...
/**
 * This class prepares the three tempFiles which are needed to calculate
 * similarities faster.
 ...
 * @version 1.0
 */
public class GrfNAVSFilePreparer
{
  public static final String ext1 = ".g1";
  public static final String ext2 = ".g2";

  protected byte[] file1 = null;
  protected byte[] file2 = null;

  public GrfNAVSFilePreparer(String fileName)
  {
  ...
  protected void writeFiles(RandomAccessFile reader, RandomAccessFile writer1, RandomAccessFile writer2) throws Exception
  {
    ...
    while ( line != null )
    {
      Integer wort_nr = new Integer( line.substring(0, line.indexOf("\t")).trim() );
      String wortart = line.substring(line.indexOf("\t"), line.lastIndexOf("\t")).trim();
      Integer grf_wort_nr = new Integer(0);
      try
      {
        grf_wort_nr = new Integer ( line.substring(line.lastIndexOf("\t"), line.length()).trim() );
      }
      catch ( Exception ex )
      {
        // just not a number
      ...
      writer1.writeBytes(wortart);
      writer1.write(Converter.IntegerToBytes(grf_wort_nr));

      if ( !curCollBlock.equals(wort_nr) )
      {
        int diff = wort_nr.intValue() - curCollBlock.intValue();
        if ( diff > 1 )
        {
  ...
  {
    GrfNAVSFilePreparer prep = new GrfNAVSFilePreparer("data/ksim/grfNAVS.dump");
  }
